...
We expect to construct a the set of  $\cal S_k = ({\bf S_M},{\bf S_M})$ for $k$
...
The General Catalog of Variable Stars (GCVS; \citep{ksf+96}) has developed a
...
1088722,CesaBianchi06,  bcmt-lmmsc-04}, with much of the interest spurred by the
problems of identifying text (e.g., \cite{593971}). We propose to explore two
...
\cite{elithesis} is to use the taxonomy to induce a taxonomically meaningful
...
classification work \cite{bcmt-lmmsc-04} might become helpful in our context. In
...
\cite{CesaBianchi06}).  We note that this sequence of classifiers could be
...
\cite{niculescu05:obtaining}.  In our context it would be desirable that the
...
\josh{Efficient implementation.  Computational issues.  Portable to LSST code
(C++ and Python).}

\medskip {\centerline{\ub{\sc Event Classification Service}}} \smallskip


\josh{An add-on to what you have running now.  Apply to PTF, Gaia.}
